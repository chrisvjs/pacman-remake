...
public class BoardMedium extends JPanel implements KeyListener, ActionListener { // Adds JFrame, Keylistener for
																					// keyboard
																					// ActionListener (to read input
																					// from keyboard)
	...
	private JLabel wonBanner = new JLabel("Game Won"); // Level Compelte Label
	...
	public void loadBoard() {
		...
		try {
			...
			input = new Scanner(new File("images/mazeMedium.txt"));
			...
			while (input.hasNext()) {
				...
				r++;
				Board.soundTrack("GAMEBEGINNING.wav");
	...
	public void keyPressed(KeyEvent key) {
		...
		if (pacMan.isDead() == false && Board.score != pellet) {
	...
	public void performMove(Mover mover) {
		...
		if (maze[mover.getNextRow()][mover.getNextColumn()] != 'W') {
			...
			if (mover == pacMan) {
				...
				animateTimer.start();
				PacManGUIMedium.lives.setText("Lives: " + Board.lives);
			...
			// replace it with a blank
				...
				if (collided() && isScared) {
					...
					for (int g = 0; g < 3; g++) {
						...
						if (ghost[g].getRow() == pacMan.getRow() && ghost[g].getColumn() == pacMan.getColumn()) {
							...
							Board.ghostEatScore += 10;
							Board.soundTrack("GHOSTEATEN.wav");
				...
				// the starting point
				else if (collided() && Board.lives > 0) {
				...
				// says the high score and plays the killed sound
					...
					Board.soundTrack("killed.wav");

					// Sets highscore equal to the score
					Board.highScore = Board.score;

					// Automatically sets the highest as the high score for the first game
					int highest = Board.highScore;

					// Make highest the new high score if it it bigger than the last
					if (Board.highScore > highest)
						highest = Board.highScore;

					// Updates the label and sets the score
					PacManGUIMedium.highScore.setText("High Score for Current Sessions: " + highest);

				}

				// Stop the game when score = pellets
				else if (thisScore == pellet) {
					stopGame();
	...
	private void moveGhosts() {
		...
		for (int x = 0; x < 3; x++) {
			...
			// because the ghosts actions are less random (1/100 chance of random movement)
	...
	private void animatePacMan() {
		...
		// set it to empty
			...
			if (maze[pacMan.getRow()][pacMan.getColumn()] == 'F' || maze[pacMan.getRow()][pacMan.getColumn()] == 'V') {
				...
				Board.score++;
				thisScore++;
				PacManGUIMedium.score.setText("Score: " + (Board.score + Board.ghostEatScore + Board.cherryEatScore));
				Board.soundTrack("pacchomp.wav");
	...
	// Displays the Game Won Banner
	private void wonBanner() {

		wonBanner.setBounds(268, 307, 800, 200);
		wonBanner.setForeground(Color.GREEN);
		wonBanner.setBackground(Color.BLACK);
		wonBanner.setFont(new Font("Comic Sans", Font.BOLD, 50));
		this.getParent().add(wonBanner);
		this.getParent().setComponentZOrder(wonBanner, 0); // Moves this to the front
		setVisible(true);
	}
